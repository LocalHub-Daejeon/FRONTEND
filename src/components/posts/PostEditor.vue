...
<template>
  <form class="post-editor modern-form" @submit.prevent="submit">
    <label class="form-group">
      <span class="label-text">{{ t("post.titleLabel") }} <small class="char-count">{{ form.title.length }}/200</small></span>
      <input
        v-model="form.title"
        class="modern-input"
        maxlength="200"
    ...
    <label class="form-group">
      <span class="label-text">{{ t("post.contentLabel") }}</span>
      <textarea
    ...
    <label class="form-group pw-group">
      <span class="label-text"><LockKeyhole :size="15" class="lock-icon" /> {{ t("post.passwordLabel") }}</span>
      <input
        ...
        :placeholder="t('post.passwordPlaceholder')"
      />
    </label>

    <div v-if="touched && !valid" class="form-error-banner">
      {{ t("post.validationError") }}
    </div>

    <div class="form-footer advanced-footer">
      <p class="notice-text">{{ t("post.guestNotice") }}</p>
      <button class="primary-button submit-btn" type="submit" :disabled="busy">
        <Save :size="18" stroke-width="2.5" />
        <span class="btn-text">{{ busy ? t("post.saving") : post ? t("post.editSubmit") : t("post.createSubmit") }}</span>
      </button>
    </div>
  </form>
</template>
